...
static file_descriptor_proto_data: &'static [u8] = b"\
    ...
    \xa2\x02\x03GPB\xaa\x02\x1eGoogle.Protobuf.WellKnownTypesJ\xe00\n\x07\
    ...
    *\x20Redistributions\x20in\x20binary\x20form\x20must\x20reproduce\x20the\
    ...
    \x12\x03(\x1b\x20\n\xb4\x1b\n\x02\x04\0\x12\x05z\0\x86\x01\x01\x1a\xa6\
    \x1b\x20A\x20Timestamp\x20represents\x20a\x20point\x20in\x20time\x20inde\
    ...
    \x20#\x20Examples\n\n\x20Example\x201:\x20Compute\x20Timestamp\x20from\
    \x20POSIX\x20`time()`.\n\n\x20\x20\x20\x20\x20Timestamp\x20timestamp;\n\
    \x20\x20\x20\x20\x20timestamp.set_seconds(time(NULL));\n\x20\x20\x20\x20\
    \x20timestamp.set_nanos(0);\n\n\x20Example\x202:\x20Compute\x20Timestamp\
    \x20from\x20POSIX\x20`gettimeofday()`.\n\n\x20\x20\x20\x20\x20struct\x20\
    timeval\x20tv;\n\x20\x20\x20\x20\x20gettimeofday(&tv,\x20NULL);\n\n\x20\
    \x20\x20\x20\x20Timestamp\x20timestamp;\n\x20\x20\x20\x20\x20timestamp.s\
    et_seconds(tv.tv_sec);\n\x20\x20\x20\x20\x20timestamp.set_nanos(tv.tv_us\
    ec\x20*\x201000);\n\n\x20Example\x203:\x20Compute\x20Timestamp\x20from\
    \x20Win32\x20`GetSystemTimeAsFileTime()`.\n\n\x20\x20\x20\x20\x20FILETIM\
    E\x20ft;\n\x20\x20\x20\x20\x20GetSystemTimeAsFileTime(&ft);\n\x20\x20\
    \x20\x20\x20UINT64\x20ticks\x20=\x20(((UINT64)ft.dwHighDateTime)\x20<<\
    \x2032)\x20|\x20ft.dwLowDateTime;\n\n\x20\x20\x20\x20\x20//\x20A\x20Wind\
    ows\x20tick\x20is\x20100\x20nanoseconds.\x20Windows\x20epoch\x201601-01-\
    01T00:00:00Z\n\x20\x20\x20\x20\x20//\x20is\x2011644473600\x20seconds\x20\
    before\x20Unix\x20epoch\x201970-01-01T00:00:00Z.\n\x20\x20\x20\x20\x20Ti\
    mestamp\x20timestamp;\n\x20\x20\x20\x20\x20timestamp.set_seconds((INT64)\
    \x20((ticks\x20/\x2010000000)\x20-\x2011644473600LL));\n\x20\x20\x20\x20\
    \x20timestamp.set_nanos((INT32)\x20((ticks\x20%\x2010000000)\x20*\x20100\
    ));\n\n\x20Example\x204:\x20Compute\x20Timestamp\x20from\x20Java\x20`Sys\
    tem.currentTimeMillis()`.\n\n\x20\x20\x20\x20\x20long\x20millis\x20=\x20\
    System.currentTimeMillis();\n\n\x20\x20\x20\x20\x20Timestamp\x20timestam\
    p\x20=\x20Timestamp.newBuilder().setSeconds(millis\x20/\x201000)\n\x20\
    \x20\x20\x20\x20\x20\x20\x20\x20.setNanos((int)\x20((millis\x20%\x201000\
    )\x20*\x201000000)).build();\n\n\n\x20Example\x205:\x20Compute\x20Timest\
    amp\x20from\x20current\x20time\x20in\x20Python.\n\n\x20\x20\x20\x20\x20t\
    imestamp\x20=\x20Timestamp()\n\x20\x20\x20\x20\x20timestamp.GetCurrentTi\
    me()\n\n\x20#\x20JSON\x20Mapping\n\n\x20In\x20JSON\x20format,\x20the\x20\
    Timestamp\x20type\x20is\x20encoded\x20as\x20a\x20string\x20in\x20the\n\
    ...
    ng\x20the\n\x20standard\x20[toISOString()](https://developer.mozilla.org\
    /en-US/docs/Web/JavaScript/Reference/Global_Objects/Date/toISOString]\n\
    \x20method.\x20In\x20Python,\x20a\x20standard\x20`datetime.datetime`\x20\
    object\x20can\x20be\x20converted\n\x20to\x20this\x20format\x20using\x20[\
    `strftime`](https://docs.python.org/2/library/time.html#time.strftime)\n\
    \x20with\x20the\x20time\x20format\x20spec\x20'%Y-%m-%dT%H:%M:%S.%fZ'.\
    \x20Likewise,\x20in\x20Java,\x20one\n\x20can\x20use\x20the\x20Joda\x20Ti\
    me's\x20[`ISODateTimeFormat.dateTime()`](\n\x20http://www.joda.org/joda-\
    time/apidocs/org/joda/time/format/ISODateTimeFormat.html#dateTime--\n\
